...
## Description 编写规范（CSO — Claude Search Optimization）

所有 `.ai/` 下的文档使用 `alwaysApply: false`（requested 模式），AI 根据 `description` 判断是否需要读取全文。因此 description 的质量直接决定了 AI 能否在正确时机加载正确文档。

### 核心原则

1. **写触发条件，不写功能摘要** — 用 "Use when..." 开头，描述什么场景下需要读取此文件
2. **列举具体触发关键词** — 把用户可能提到的术语、类名、概念都写进去，提高命中率
3. **包含同义词和症状** — 用户不一定用技术术语描述问题，要覆盖常见表述方式
4. **禁止写流程摘要** — 不要让 AI 看了 description 就觉得"够了不用读全文"

### Checklist

新增或修改 `.ai/` 文档时，确认 description 满足：
- [ ] 以 "Use when" 开头
- [ ] 列出至少 3 个触发场景
- [ ] 包含文档中的核心技术关键词（类名、API 名、工具名）
- [ ] 不含步骤性描述（"第一步...第二步..."）
- [ ] 英文撰写（AI 模型对英文触发词匹配更准确）
